...
public class BarangMasukQueries {
    ...
    public final static String GET_DATA_BARANG_MASUK = "SELECT bm.id, u.username AS user, d.nama AS distributor, bm.tgl_masuk, bm.jumlah, s.status AS status FROM barang_masuk AS bm"
    ...
    public final static String ADD_BARANG_MASUK = "INSERT INTO barang_masuk (id_user, id_distributor, tgl_masuk, jumlah)"
            + "VALUES (?,?,?,?)";
    public final static String GET_SINGLE_DATA = "SELECT bm.id, bm.tgl_masuk AS tanggal, bm.jumlah AS jumlah, u.nama AS user, "
